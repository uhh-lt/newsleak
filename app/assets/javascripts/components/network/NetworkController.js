            ...
            [
                ...
                )
                {
                    ...
                    if (document.cookie){
                        ...
                        var fontscale = parseInt(cookieArray[0]);
                        var fontDyOffset = parseInt(cookieArray[1]);
                    }
                    ...
                    toolShareService.updateGuidance = function () {
                        if (toolShareService.UIitemsChanged) {
                            ...
                            //document.getElementById("settings-button").className += " collapsed";
                            $("#demo").collapse("hide");
                            getGuidanceNodes(oldFocusNodeId, false, true);
                        }
                    };
                    ...
                    function createSVG(){
                        ...
                        d3.select('body').on('keydown', function(){
                            ...
                            d3.selectAll(".node").select("text").style("font-size",10+fontscale).attr("dy",fontDyOffset);
                            document.cookie = fontscale+","+fontDyOffset;
                    ...
                    function start(callback){//Graph zeichnen
                        ...
                        newNodes.append('text')
                            ...
                            .style('font-size', 10+fontscale /*function(d){
                             var r = radius(d.docCount);
                             var textLength;
                             var size;
                             if(r/3 >= d.name.length - 1){  // If the text fits inside the node.
                        ...
                        playRoutes.controllers.NetworkController.undo(sessionid).get().then(
                            function (response) {
                                ...
                                for(var i = hist.length-1; i>=0; i--){
                                    switch (state) {
                                        case "s-start":
                                            if (["guidance","prioChange"].includes(hist[i].type) && hist[i].active) {
                                                hist[i].active = false;
                                                hist[i].undone = true;
                                                state = "s1";
                                                undoList.push(i);
                                        ...
                                        case "s1":
                                            ...
                                            else if (["guidance","prioChange"].includes(hist[i].type) && !hist[i].undone){
                                                hist[i].active = true;
                                                state = "s3";
                                                undoList.push(i);
                                            }
                        ...
                        playRoutes.controllers.NetworkController.redo(sessionid).get().then(
                            function (response) {
                                ...
                                for (var i = 0; i<undoObj.list.length; i++){
                                    ...
                                    if (["guidance","prioChange"].includes(item.type)){
                    ...
                    function getGuidanceNodes(focusNodeId,useOldEdges,prioChange){
                        prioChange = typeof prioChange !== 'undefined' ? prioChange : false;
                        ...
                        playRoutes.controllers.NetworkController.getGuidanceNodes(fulltext, facets, entities, $scope.observer.getTimeRange(), focusNodeId, toolShareService.sliderEdgeAmount(), toolShareService.sliderEdgesPerNode(), uiStr, useOldEdges, sessionid).get().then(function(response) {
                            ...
                            $scope.observer.addItem(new function (){
                                this.type = (prioChange?"prioChange":"guidance");
                                this.data = (prioChange?{name:"changed prios"}:nodes.find(function(node){return focusNodeId === node.id}));
                            });
                            start();
                            $scope.isViewLoading = false;
                            $scope.contextCache = {};
